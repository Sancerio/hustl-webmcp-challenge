...
void registerDemoDependencies(
  ...
  final state = DemoState(allowWebMcpFoodAutoLog: !challengeMode);
  ...
  // — keeps this demo repo instead of the backend-backed Api implementation that
  // errors offline. Ordinary demo mode keeps the illustrative seeded inbox;
  // the challenge begins empty so every visible proposal comes from the live
  // AI collaboration being evaluated.
  put<ProposalsRepository>(
    DemoProposalsRepository(
      anchor: resolvedAnchor,
      seedInitialProposals: !challengeMode,
      state: state,
